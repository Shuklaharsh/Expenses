...
def edit_expense(request,id):
    ...
    if request.method == "POST":
        ...
        if not amount.isnumeric():
            messages.error(request,'Amount msut be number')
            return render(request, 'expenses/edit_expense.html',context)
...
def export_csv(request):
    ...
    response['Content-Deposition'] = 'attachment; filename=Expenses' + \
...
def export_excel(request):
    ...
    response['Content-Deposition'] = 'attachment; filename=Expenses' + str(datetime.datetime.now()) +'.xls'
